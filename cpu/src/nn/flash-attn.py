...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, o, L, causal, sm_scale):
        ...
        q_torch = torch.as_tensor(q, dtype=torch.float32, device="cuda")
        k_torch = torch.as_tensor(k, dtype=torch.float32, device="cuda")
        v_torch = torch.as_tensor(v, dtype=torch.float32, device="cuda")
        o_torch = torch.as_tensor(o, dtype=torch.float32, device="cuda")
        L_torch = torch.as_tensor(L, dtype=torch.float32, device="cuda")
...
if __name__ == "__main__":
    ...
    q = torch.randn(16, 8, 128, 64, device="cuda", dtype=torch.float32)
    k = torch.randn(16, 8, 128, 64, device="cuda", dtype=torch.float32)
    v = torch.randn(16, 8, 128, 64, device="cuda", dtype=torch.float32)
